...
public class HostPatch_Start
{
    ...
    {
        Plugin.Log.LogInfo("Patching Host.Start");
        ...
        return Plugin.PatchFieldLoad(startParametersFieldInfo, abilityFieldInfo, multiStartParametersFieldInfo,
...
public class HostPatch_ReInit
{
    public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
    {
        Plugin.Log.LogInfo("Patching Host.ReInit");
        FieldInfo startParametersFieldInfo =
            AccessTools.Field(typeof(SteamManager), nameof(SteamManager.startParameters));
        FieldInfo multiStartParametersFieldInfo =
            AccessTools.Field(typeof(SteamManagerExtended), nameof(SteamManagerExtended.startParameters));
...
public class HostPatch_Init
{
    ...
    {
        Plugin.Log.LogInfo("Host::Init::Prefix");
...
public class HostPatch_ProcessNetworkPackets
{
    ...
    {
        Plugin.Log.LogInfo("Adding default packet");
    ...
    {
        Plugin.Log.LogInfo("Patching Host.ProcessNetworkPackets");
        ...
        while (enumerator.MoveNext())
        {
            ...
            if (instruction.opcode == OpCodes.Ldloca_S)
            {
                ...
                {
                    Plugin.Log.LogError("Expected to find next instruction after Ldloca_S");
                    yield return loopStart;
                    yield break;
                }
                ...
                {
                    Plugin.Log.LogInfo("Found possible start of loop. Searching for end...");
                    bool foundEnd = false;
                    ...
                    while (enumerator.MoveNext())
                    {
                        ...
                        {
                            Plugin.Log.LogInfo("Found End of loop");
                            foundEnd = true;
                            break;
                        }
                    }
                    ...
                    if (foundEnd)
                    {
                        if (last_br == null)
                        {
                            Plugin.Log.LogError("What this shouldn't be null?!");
                        }
                        
                        // Check which loop was found
                        if (instructionsSkipped.Any(i => i.LoadsField(delayBufferSizeFieldInfo)))
                        {
                            Plugin.Log.LogInfo("Found 'fill InputBuffer with default InputPacket' loop");
                        ...
                        else if (instructionsSkipped.Any(i => i.LoadsField(updatesPlacedInDelayBufferFieldInfo)))
                        {
                            ...
                            Plugin.Log.LogInfo($"Local_i: {local_i}");
                            Plugin.Log.LogInfo($"Local_m: {local_m}");
                            
                            Plugin.Log.LogInfo("Found InputPacket creation loop");
...
public class HostPatch_Update
{
    ...
    public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions,
        ILGenerator generator)
    {
        Plugin.Log.LogInfo("Patching Host.Update");
        ...
        while (enumerator.MoveNext())
        {
            ...
            {
                Plugin.Log.LogInfo("Found InputBuffer field load instruction");
                foundInputBuffer = true;

                if (!enumerator.MoveNext())
                {
                    Plugin.Log.LogError("Expected to find next instruction after InputBuffer field load instruction");
                    yield return instruction;
                    continue;
                }
                ...
                {
                    Plugin.Log.LogInfo("Patching InputBuffer.getCount");
                ...
                {
                    Plugin.Log.LogInfo("Patching InputBuffer.Dequeue");
                    ...
                    {
                        Plugin.Log.LogError("Expected a store local instruction.");
                    ...
                    while (enumerator.MoveNext())
                    {
                        ...
                        if (instruction.opcode == OpCodes.Ldarg_0)
                        {
                            if (!enumerator.MoveNext())
                            {
                                Plugin.Log.LogError("Expected to find next instruction after Ldarg_0");
                                yield return instruction;
                                continue;
                            }
                            ...
                            {
                                Plugin.Log.LogInfo("Found AdaptiveInputDelayBuffer check");
                                yield return new CodeInstruction(OpCodes.Ldarg_0);
                                yield return instruction;

                                if (!enumerator.MoveNext())
                                {
                                    Plugin.Log.LogError("Invalid input instruction stream.");
                                    continue;
                                }
                ...
                Plugin.Log.LogError("Failed to patch InputBuffer reference out.");
            }

            if (foundInputBuffer &&
                instruction.Calls(AccessTools.Method(typeof(PlayerHandler), nameof(PlayerHandler.Get))))
            {
                Plugin.Log.LogInfo("Patching PlayerHandler.Get calls");
                ...
                {
                    Plugin.Log.LogError("Failed to find previousInputQuad store");
                    yield break;
                }
