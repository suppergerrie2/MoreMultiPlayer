...
namespace MoreMultiPlayer
{
    ...
    public static class CharacterSelecterHandler_online_ForceStartGamePatch
    {
        ...
        public static bool Prefix([CanBeNull] PlayerColors pcs)
        {
            ...
            Plugin.Log.LogInfo($"SteamID: {SteamClient.SteamId}");

            for (int i = 0; i < startParameters.nrOfPlayers; i++)
            {
                Plugin.Log.LogInfo($"Initializing player {i}: [id: {startParameters.p_ids[i]}, color: {startParameters.p_colors[i]}, team: {startParameters.p_teams[i]}, ability1: {startParameters.p_ability1s[i]}, ability2: {startParameters.p_ability2s[i]}, ability3: {startParameters.p_ability3s[i]}]");
            ...
            else
            {
                ...
                Plugin.Log.LogInfo("SteamIdIndex: " + steamIdIndex);
                if (steamIdIndex != -1)
                {
            ...
            if (player == null)
            {
                Plugin.Log.LogError("Failed to find local player on ForceStart.");
                return false;
            }
