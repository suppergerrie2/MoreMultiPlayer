...
public static class NetworkToolsExtensions
{
    ...
    {
	    Plugin.Log.LogInfo($"Decoding MultiStartRequestPacket, size of array: {data.Length}");
		...
		Plugin.Log.LogInfo($"Decoded seqNum: {result.seqNum} (byte: {num})");
		...
		Plugin.Log.LogInfo($"Decoded seed: {result.seed} (byte: {num})");
		...
		Plugin.Log.LogInfo($"Decoded nrOfPlayers: {result.nrOfPlayers}, nrOfAbilities: {result.nrOfAbilites}, currentLevel: {result.currentLevel}, frameBufferSize: {result.frameBufferSize}, isDemoMask: {result.isDemoMask} (byte: {num})");
		...
		for (int i = 0; i < result.nrOfPlayers; i++)
		{
			...
			Plugin.Log.LogInfo($"Decoded p_ids[{i}]: {result.p_ids[i]} (byte: {num})");
		}

		for (int i = 0; i < result.nrOfPlayers; i++)
		{
			result.p_colors[i] = data[num++];
			Plugin.Log.LogInfo($"Decoded p_colors[{i}]: {result.p_colors[i]} (byte: {num})");
		}

		for (int i = 0; i < result.nrOfPlayers; i++)
		{
			result.p_teams[i] = data[num++];
			Plugin.Log.LogInfo($"Decoded p_teams[{i}]: {result.p_teams[i]} (byte: {num})");
		}

		for (int i = 0; i < result.nrOfPlayers; i++)
		{
			result.p_ability1s[i] = data[num++];
			Plugin.Log.LogInfo($"Decoded p_ability1s[{i}]: {result.p_ability1s[i]} (byte: {num})");
		}

		for (int i = 0; i < result.nrOfPlayers; i++)
		{
			result.p_ability2s[i] = data[num++];
			Plugin.Log.LogInfo($"Decoded p_ability2s[{i}]: {result.p_ability2s[i]} (byte: {num})");
		}

		for (int i = 0; i < result.nrOfPlayers; i++)
		{
			result.p_ability3s[i] = data[num++];
			Plugin.Log.LogInfo($"Decoded p_ability3s[{i}]: {result.p_ability3s[i]} (byte: {num})");
		}
    ...
    {
	    Plugin.Log.LogInfo($"Encoding MultiStartRequestPacket, size of array: {data.Length}");
	    int num = 0;
	
	    Plugin.Log.LogInfo($"Encoding seqNum: {p.seqNum} (byte: {num})");
	    p.seqNum = NetworkTools.SwapBytesIfLittleEndian(p.seqNum);
		byte[] bytes = BitConverter.GetBytes(p.seqNum);
		data[num++] = bytes[0];
		data[num++] = bytes[1];
		
		Plugin.Log.LogInfo($"Encoding seed: {p.seed} (byte: {num})");
		...
		Plugin.Log.LogInfo($"Encoded nrOfPlayers: {p.nrOfPlayers}, nrOfAbilities: {p.nrOfAbilites}, currentLevel: {p.currentLevel}, frameBufferSize: {p.frameBufferSize}, isDemoMask: {p.isDemoMask} (byte: {num})");

		for (int i = 0; i < p.nrOfPlayers; i++)
		{
			Plugin.Log.LogInfo($"Encoding p_ids[{i}]: {p.p_ids[i]} (byte: {num})");
		...
		for (int i = 0; i < p.nrOfPlayers; i++)
		{
			...
			Plugin.Log.LogInfo($"Encoded p_colors[{i}]: {p.p_colors[i]} (byte: {num})");
		}
		...
		for (int i = 0; i < p.nrOfPlayers; i++)
		{
			...
			Plugin.Log.LogInfo($"Encoded p_teams[{i}]: {p.p_teams[i]} (byte: {num})");
		}
		
		for (int i = 0; i < p.nrOfPlayers; i++)
		{
			data[num++] = p.p_ability1s[i];
			Plugin.Log.LogInfo($"Encoded p_ability1s[{i}]: {p.p_ability1s[i]} (byte: {num})");
		}
		
		for (int i = 0; i < p.nrOfPlayers; i++)
		{
			data[num++] = p.p_ability2s[i];
			Plugin.Log.LogInfo($"Encoded p_ability2s[{i}]: {p.p_ability2s[i]} (byte: {num})");
		}
		
		for (int i = 0; i < p.nrOfPlayers; i++)
		{
			data[num++] = p.p_ability3s[i];
			Plugin.Log.LogInfo($"Encoded p_ability3s[{i}]: {p.p_ability3s[i]} (byte: {num})");
		}
    }
